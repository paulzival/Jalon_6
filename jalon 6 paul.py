...
def chercher_balle_G():
       cmd_moteur(0,70,0)

def chercher_balle_D():
       cmd_moteur(0,0,70)
...
def chercher_sol_bleu(img):
    ...
    if blobs_bleus:
      # tris de la taille des blobs
        ...
        if largest_blue.cx() > img.width() // 2 + 20:  # marge d'éreur
            chercher_balle_G()
        else:
            chercher_balle_D()
...
while True:
    ...
    if Cd.value() == 1:
        etat = "fourche"
    elif Dist < 12 and etat != "recherche_bleu":  # Priorité au mur sauf si en recherche_bleu
        etat = "mur"
