...
def chercher_sol_bleu(img):
    ...
    if blobs_bleus:
        largest_blue = max(blobs_bleus, key=lambda b: b.pixels())
        if largest_blue.cx() < img.width() // 2:
            code = 1
            M11.pulse_width_percent(-80)  # Tourner à gauche
            M12.pulse_width_percent(80)

        else:
            code = 2
            M11.pulse_width_percent(80)   # Tourner à droite
            M12.pulse_width_percent(-80)
