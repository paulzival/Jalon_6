...
def chercher_balle():
    if code==2:
        cmd_motor(0,70,0) # Tourner a droite
    if code == 1:
        cmd_motor(0,0,70) # Tourner a gauche


def cmd_motor(av_ar,mg,md):
...
def suivre_balle(blob_cx, blob_cy, img_width):
    ...
    if delta_x < 0:  # Balle à droite
        M11.pulse_width_percent(max(20, vitesse_base - ajustement))
        M12.pulse_width_percent(min(100, vitesse_base + ajustement))
...
def chercher_sol_bleu(img):
    global etat
    #blobs_bleus = img.find_blobs([thresholds_bleu], roi=(0, img.height()//2, img.width(), img.height()//2))
    if blobs_bleus:
        sx = sol[-1].cx()
        ...
        M11.pulse_width_percent(50)  # Tourner lentement pour chercher
        M2x.pulse_width_percent(50)
        time.sleep_ms(200)
...
while True:
    ...
    blobs_bleus = img.find_blobs([thresholds_bleu],pixel_,area_threshold=100 ,merge=True)
